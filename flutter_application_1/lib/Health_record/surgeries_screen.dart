...
class _SurgeriesScreenState extends State<SurgeriesScreen> {
  ...
  String? _selectedSurgery; // اختيار واحد فقط
  ...
  Future<void> _saveSurgery() async {
    if (_selectedSurgery == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('يرجى اختيار العملية الجراحية')));
      return;
    }
    
    String surgery = _selectedSurgery!;
    if (_isOtherSelected) {
       if (_otherSurgeryController.text.trim().isEmpty) {
         ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('يرجى كتابة اسم العملية')));
         return;
       }
       surgery = _otherSurgeryController.text.trim();
    }
    ...
    try {
      ...
      if (user != null) {
        ...
        if (!profileProvider.hasPermission('surgeries', 'write')) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('عذراً، ليس لديك صلاحية لإضافة عمليات جراحية'),
        ...
        final collectionPath = profileProvider.getFirestorePath(user.uid, 'surgeries');

        await FirebaseFirestore.instance.collection(collectionPath).add({
          'surgery_name': surgery,
          'surgery_date': _selectedDate,
  ...
  Widget _buildSurgeriesExpansionTile() {
    return Container(
      ...
      child: Theme(
        ...
        child: Directionality(
          ...
          child: ExpansionTile(
            ...
            title: const Text(
              'اختر العملية الجراحية التي أجريتها',
            ...
            children: [
              Padding(
                ...
                child: Column(
                  children: _surgeries.map((surgery) {
                    final isOther = surgery == 'أخرى';
                    return Column(
                      children: [
                        RadioListTile<String>(
                          ...
                          value: surgery,
                          groupValue: _selectedSurgery,
                          onChanged: (String? value) {
                            setState(() {
                              _selectedSurgery = value;
                              _isOtherSelected = value == 'أخرى';
                              if (!_isOtherSelected) {
                                _otherSurgeryController.clear();
                              }
                            });
                          },
