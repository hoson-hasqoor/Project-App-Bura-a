...
class _HospitalAdmissionScreenState extends State<HospitalAdmissionScreen> {
  ...
  Future<void> _saveHospitalAdmission() async {
    if (_selectedReason == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('يرجى اختيار سبب الدخول')));
      return;
    }
    ...
    if (reason == 'أخرى') {
      if (_otherReasonController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('يرجى كتابة السبب الآخر')));
        return;
      }
    ...
    try {
      ...
      if (user != null) {
        ...
        if (!profileProvider.hasPermission('hospital_stays', 'write')) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('عذراً، ليس لديك صلاحية لإضافة دخول مستشفى'),
        ...
        final collectionPath = profileProvider.getFirestorePath(user.uid, 'hospital_stays');

        await FirebaseFirestore.instance.collection(collectionPath).add({
          'admission_reason': reason,
          'admission_date': _selectedAdmissionDate,
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: _isLoading ? null : _saveHospitalAdmission,
